...
@Author  : ZhangShenao
@File    : gen_video_with_first_frame.py
@Desc    : 使用首帧作为参考图，生成视频
"""
...
def generate_video(
    ...
    """
    try:
        url = f"{API_BASE_URL}/openapi/v2/video/img/generate"
        headers = {
            "API-KEY": API_KEY,
            "Ai-trace-id": trace_id,
...
def main():
    ...
    image_path = current_dir / "first_frame.png"

    # 步骤1: 上传图片
    print("\n=== 步骤1: 上传参考图片 ===")
    img_id = upload_image(str(image_path), trace_id)

    if img_id is None or img_id is None:
        print("错误: 图片上传失败，无法继续")
        return

    # 步骤2: 调用Gen Video API生成视频
    print("\n=== 步骤2: 提交视频生成任务 ===")
